...
//   AC-43 coverage: an index the root index does not reach may not vouch for anything. `index`
//              writes but never deletes, so narrowing what gets indexed strands an index.md on disk
//              with its rows intact — and counting those rows credits a document nothing a reader
//              can follow leads to. CANARY: a broken chain leaves the documents named and the
//              stranded generated index reported as orphan-index. Controls: repairing the chain
...
// ---------- AC-43 an index nothing links to may not vouch for a document ----------
// The quiet half of the same fail-open family. An orphan is not an unreached document, it is a
// reached *nothing*: coverage reports zero findings whether the debris is there or not, so the
// only way anyone found this class was diffing the file list across two versions of the tool.
// Reported from claude-code-config after FR-OKF-4's payload pruning stranded eight index files.
const MARKER = '<!-- generated by okf.mjs - do not edit; regenerate after changing frontmatter -->';
R = repo('orphan', { unreachable: false });
describeAll(R);
git(R, 'add', '-A');
eq('AC-43 control: the chained tree starts clean', run('coverage', R).rc, 0);

// break the chain the way narrowing does: a regenerated root that no longer links docs/
const rootOnly = `---\nokf_version: "0.2"\n---\n\n${MARKER}\n\n# Other\n\n* [Project](README.md)\n`;
write(join(R, 'index.md'), rootOnly);
res = run('coverage', R);
eq('AC-43 canary: a stranded index no longer covers its documents', res.rc, 1);
has('AC-43 and they are named', res.out, 'unindexed: docs/requirements/FR-001.md');
has('AC-43 the stranded index is named as debris', res.out, 'orphan-index: docs/index.md');
has('AC-43 with the reason it goes stale unseen', res.out, 'no longer maintains them');
has('AC-43 and the remedy', res.out, 'delete them');

// control: repairing the chain clears the documents AND the orphan report together
describeAll(R);
res = run('coverage', R);
eq('AC-43 control: repairing the chain clears it', res.rc, 0);
hasNot('AC-43 control: and nothing is called debris any more', res.out, 'orphan-index:');

// control: a stray this tool did not write is somebody else's file, never named as its debris —
// its rows still do not count, so the documents below it are reported either way
write(join(R, 'index.md'), rootOnly);
write(join(R, 'docs/index.md'), '# Docs\n\n* [Requirements](requirements/index.md)\n');
res = run('coverage', R);
